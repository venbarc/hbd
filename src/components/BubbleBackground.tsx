...
// Use fewer bubbles on mobile to save GPU compositing layers
const mobileBubbles = allBubbles.filter((_, i) => i % 3 === 0) // 4 bubbles

export default function BubbleBackground() {
  const [bubbles, setBubbles] = useState(allBubbles)

  useEffect(() => {
    if (window.innerWidth < 640) setBubbles(mobileBubbles)
  }, [])
